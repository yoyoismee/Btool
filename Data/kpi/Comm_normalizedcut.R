comm_normalizedcut <- function(graph, membership){
    ...
    #graph_table
    normalized_cut <- mean(graph_table$cs/(graph_table$cs+2*graph_table$ms) + 
                              graph_table$cs/(graph_table$cs+2*(length(E(graph)) - graph_table$ms)))
